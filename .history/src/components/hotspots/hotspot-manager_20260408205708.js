class HotspotManager {
    ...
    listenEvents() {
        ...
        this.events.on('hotspot:hide-all', (data) => {
            this.activeData = null
            this.activeHotspot = null
            this.update(true)
            this.hideAll()
    ...
    setActive(hotspot, lerpDuration = 1.5) {
        ...
        console.log('is same pose?', isSamePose)
        if (isSamePose && hotspot.id === this.activeHotspot?.id) {
            console.log('already active, just show')
            hotspot.show()
            return
        }
        if (isSamePose) {
            console.log("run instantly because it's the same pose")
            hotspot.show()
    ...
    hideAll() {
        console.log('hiding all hotspots')
